...
state BellmanFord2(struct route_node **local, struct route_node **remote, int remotert){
  int i;
  //int newCost;
  enum state status = NOTUPDATED;
  //enum state status = NOTUPDATED;
  //printf("%s",printTable(local,remotert));
  for(i=0;i<N;i++){
    if(local[i]->interface=='l'||remote[i]->interface=='l') continue;
    //printf("remotert = %d\n",remotert);
    //printf("remote[%d]->cost = %d\n",i,remote[i]->cost);
    //printf("local[%d]->cost = %d\n",remotert,local[remotert]->cost);
    int newCost = remote[i]->cost+local[remotert]->cost;
    //printf("new cost = %d\n",newCost);
    //printf("local cost = %d\n",local[i]->cost);
    if(local[i]->cost > newCost) {
      printf("Updating : dest = %d to cost = %d via iface = %c\n",local[i]->dest,newCost,local[remotert]->interface);
      //printf("updating local 
      updateRoute(local,local[i]->dest,newCost,local[remotert]->interface);
      status = UPDATED;
    }
  }
