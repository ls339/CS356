...
void updateCost(struct route_node **tb, int dest, int cost) {
  if(tb[dest]->interface!='l') tb[dest]->cost = cost;
}
...
char * printTable2(struct route_node **tb,int routerno) {
  char *str = (char *) malloc(1024);
  int i;
  sprintf(str,"+--------------------------------------------+\n");
  sprintf(str + strlen(str),"| Router %d                                   |\n",routerno);
